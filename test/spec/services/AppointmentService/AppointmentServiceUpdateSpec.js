describe('Service: AppointmentServiceUpdateSpec', function() {
	...
	it('should update a appointment instance', function() {
		...
		AppointmentKeeper.update = jasmine
				.createSpy('AppointmentKeeper.update');
		AppointmentService.isValid = jasmine.createSpy(
				'AppointmentService.isValid').andReturn([]);
	...
	it('shouldn\'t update a appointment instance', function() {
		// given
		AppointmentService.isValid = jasmine.createSpy(
				'AppointmentKeeper.isValid').andReturn([]);
		AppointmentKeeper.update = jasmine
	...
	it('should done an appointment', function() {
		...
		AppointmentService.isValid = jasmine.createSpy(
				'AppointmentKeeper.isValid').andReturn([]);
		AppointmentKeeper.list = jasmine.createSpy('AppointmentKeeper.list')
	...
	it('should cancel an appointment', function() {
		...
		// then
		expect(AppointmentKeeper.update)
				.toHaveBeenCalledWith(appointmentCancel);
		expect(result).toBe(undefined);
	});
	...
	it('should remove an appointment', function() {
		...
		// then
		expect(AppointmentKeeper.update).toHaveBeenCalledWith(
				appointmentRemoved);
		expect(result).toBe(undefined);
	});
