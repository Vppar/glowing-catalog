describe(
		'Service: AppointmentServiceListSpec',
		function() {
			...
			it('should return full list copy', function() {
				...
				AppointmentKeeper.list = jasmine.createSpy(
						'AppointmentKeeper.list').andReturn(dummyReceivables);
			...
			it(
					'shouldn\'t return full list copy',
					function() {
						// given
						AppointmentKeeper.list = jasmine.createSpy(
								'AppointmentKeeper.list').andCallFake(
								function() {
									throw 'my exception';
								});
						...
						expect(log.debug)
								.toHaveBeenCalledWith(
										'AppointmentService.list: Unable to recover the list of appointments. Err=my exception');
			...
			it('should list since 01/01/2000 upon 05/01/2000', function() {
				...
				upon.setMinutes(0);
				// when
				var appointments = AppointmentService.listAppointmentsByPeriod(
						since, upon);
