...
describe('Service: Coupon', function() {
	...
	it('should create a coupon', function() {
		...
		expect(voucherStub.create.mostRecentCall.args[0].entity)
				.toEqual(entity);
		expect(voucherStub.create.mostRecentCall.args[0].amount)
				.toEqual(amount);
		expect(voucherStub.create.mostRecentCall.args[0].remarks).toEqual(
				remarks);
		expect(voucherStub.create.mostRecentCall.args[0].document).toEqual(
				document);
	...
	it('should not create a coupom without entity', function() {
		...
		expect(function() { CouponService.create(undefined, amount, remarks, document)}).toThrow();
		expect(voucherStub.create).not.toHaveBeenCalled();
