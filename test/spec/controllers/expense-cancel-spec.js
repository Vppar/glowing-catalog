describe('Controller: ExpenseCtrl', function() {
   ...
   /** 
    * Given a fulfilled expense
    * when a cancel is triggered
    * then the the expense must not be canceled
    * and a message should be logged: "unable to cancel an already fulfilled expense"
    */
    it('shouldn\'t cancel a fulfilled expense', function() {
        // given
        scope.expense.id = expenseId;
        scope.expense.payed = {date: 1385380800000, amount: '100.0'};
        scope.expense.canceled = false;
        ...
        expect(log.error).toHaveBeenCalledWith('ExpenseCtrl: -Unable to cancel an already fulfilled expense.');
