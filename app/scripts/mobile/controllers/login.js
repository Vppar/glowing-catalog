  ...
  angular.module('tnt.catalog.login.ctrl', [
      ...
      UserService.hasUnsyncedData().then(function (hasUnsyncedData) {
        ...
        if (hasUnsyncedData) {
          ...
          DialogService.messageDialog({
            ...
            message: 'Alguns dados não foram salvos no servidor e seriam excluídos permanentemente. ' +
            'Para trocar de usuário, faça um login on-line para que dados possam ser salvos e tente novamente.',
            btnYes: 'Fechar'
          }).then(function () {
            $log.debug('Aborted, user has unsynced data.');
          });
        } else {
    ...
    $scope.login = function () {
      return UserService.login($scope.user, $scope.pass, $scope.rememberMe).then(function () {
        ...
        DialogService.openDialogLoginFailed({
          title: 'Login',
          message: msg,
          btnYes: 'Voltar'
        });
