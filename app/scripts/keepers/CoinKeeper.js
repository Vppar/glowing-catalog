(function(angular) {
    ...
    angular.module('tnt.catalog.coin.entity', [
        'tnt.identity', 'tnt.catalog.journal'
    ]).factory(
            'Coin',
            function Coin() {

                var service =
                        function svc(uuid, created, entityId, amount, duedate) {

                            var validProperties =
                                    [
                                        'uuid', 'created', 'entityId', 'documentId', 'type', 'payment', 'amount', 'duedate', 'canceled',
                                        'liquidated', 'remarks', 'document'
                                    ];
    ...
    angular.module(
            ...
            [
                ...
                'Replayer',
                'CheckPayment',
                function CoinKeeper(ArrayUtils, Coin, IdentityService, JournalKeeper, JournalEntry, Replayer, CheckPayment) {
                    ...
                    function instance(name) {
                        ...
                        ObjectUtils.ro(this.handlers, 'receivableAddV1', function(event) {
                            ...
                            return _this.handlers.receivableAddV2(event);
                        });

                        /**
                         * Registering handlers
                         */
                        ObjectUtils.ro(this.handlers, name + 'AddV2', function(event) {
                            // Get the coin info from type map, get the
                            // respective entity
                            // and instantiate
                        ...
                        ObjectUtils.ro(this.handlers, name + 'LiquidateV1', function(event) {
                            var coin = ArrayUtils.find(vault, 'uuid', event.uuid);
                            if (coin) {
                                // Get the coin info from type map and get the
                                // respective
                        ...
                        var add = function add(coin) {
                            ...
                            var entry = new JournalEntry(null, event.created, name + 'Add', 2, event);
                         ...
                         * @param executionDate - Date that the coin was
                         *            executed(payed or received).
                         */
                        ...
                        var update =
                                function(coin) {
                                    ...
                                    var entry =
                                            new JournalEntry(
                                                    null, receivable.created, name + 'UpdatePayment', currentEventVersion, receivable);
                                    // save the journal entry
                                    return JournalKeeper.compose(entry);
                        ...
                        this.updateReceivable = function(receivable) {
                            ...
                            // create a new journal entry
                            var entry = new JournalEntry(null, stamp, 'updateReceivable', currentEventVersion, event);
                        ...
                        var updateCheck =
                                function(check) {
                                    ...
                                    // FIXME - quick fix to remove the old and
                                    // useless check.id
                                    if (check.id) {
                                        delete check.id;
                                    }
