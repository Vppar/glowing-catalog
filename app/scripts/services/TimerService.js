(function(angular) {
    ...
    angular.module('tnt.catalog.timer.service', []).service(
        ...
        function TimerService($log) {
            ...
            Timer.prototype =
                {
                     ...
                     * 
                     * @return {number} If the timer has not been started,
                     *         returns 0. If started and stopped, returns the
                     *         difference between both timestamps. If still
                     *         running, returns the time in milliseconds since
                     *         the timer started.
                     */
                    getElapsedTime : function( ) {
                        ...
                        if (this.started) {
                            $log.debug('Getting elapsed time for a running timer (' + this.id +
                                ').');
                    ...
                    getStatus : function( ) {
                        ...
                        if (this.started && this.stopped) {
                            return description ? 'Took ' + elapsedTime + 'ms to ' + description +
                                ' (' + id + ').' : 'Timer ' + id + ' runned for ' + elapsedTime +
                                'ms.';
                        }

                        if (this.started) {
                            return description ? 'Took ' + elapsedTime + 'ms to ' + description +
                                ' (' + id + '). Still running.' : 'Timer ' + id + ' runned for ' +
                                elapsedTime + 'ms and is still running.';
                        }
            ...
            /**
             * Creates and starts a timer. If a timer with the given id already
             * exists, it won't do anything.
             * 
            ...
            /**
             * Stops the timer with the given id. If no timer is found, it will
             * do nothing. If timer is already stopped, it will be removed from
             * the list.
             * 
            ...
            this.stopTimer =
                function(id) {
                    ...
                    if (!timer) {
                        $log.debug('Unable to find a timer with the given id (' + id + '). ' +
