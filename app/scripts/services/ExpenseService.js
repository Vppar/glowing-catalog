(function(angular) {
    ...
    angular.module('tnt.catalog.service.expense', [
        'tnt.catalog.expense.entity', 'tnt.catalog.receivable.keeper'
            ...
            'ExpenseService',
            function ExpenseService($log, CoinKeeper) {
                ...
                /**
                 * Verify if a expense is valid and register in the
                 * datastore.
                 * 
                 * @param expense - expense object to be registered.
                 * @return boolean - Result if the expense was accepted or
                 *         declined.
                 */
                var register =
                        function register(expense) {
                            ...
                            try {
                                ...
                                result = false;
                                $log.debug('ExpenseService.register: Unable to register a expense=' + JSON.stringify(expense) +
                                    ', ' + err);
                            }
                            return result;
                        };
                 ...
                 */
                var pay =
                        function pay(id, dueDate) {
                            var result = true;
                            try {
                                ExpenseKeeper.receive(id, dueDate);
                            } catch (err) {
                                result = false;
                                $log.debug('ExpenseService.register: Unable to make the payment of expense=' +
                                    JSON.stringify(expense) + ', ' + err);
                            }
                            return result;
                        };
