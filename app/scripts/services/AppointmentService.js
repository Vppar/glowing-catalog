(function (angular) {
    ...
    angular
        ...
        .service(
            'AppointmentService',
            function AppointmentService ($log, $q, AppointmentKeeper, Appointment) {
                ...
                this.isValid = function (entity) {
                    ...
                    invalidProperty.dataInicial = angular.isDefined(entity.startDate);
                    invalidProperty.dataFinal = angular.isDefined(entity.endDate);
                ...
                this.list =
                    function () {
                        ...
                        try {
                            ...
                            $log
                                .debug('AppointmentService.list: Unable to recover the list of appointments. Err=' +
                                    err);
                        }
                        return result;
                    };

                this.listAppointmentsByPeriod =
                    function (since, upon, types) {
                        ...
                        if (appointmentList) {
                            for ( var idx in appointmentList) {
                                ...
                                if (appointment.status !== 'REMOVED') {
                                    ...
                                    if (dateAppointment.getTime() >= since.getTime() &&
                                        dateAppointment.getTime() <= upon.getTime()) {
                                        if (!types) {
                ...
                this.read =
                    function (uuid) {
                        ...
                        try {
                            ...
                            $log
                                .debug('AppointmentService.read: Unable to find an appointment with id=\'' +
                                    uuid + '. Err=' + err);
                        }
                        return result;
                    };
                ...
                this.done =
                    function (uuid) {
                        ...
                        var appointment = '';
                        try {
                            appointment = this.loadByUUID(uuid);
                            appointment.status = 'DONE';
                            result = this.update(appointment);
                        } catch (err) {
                            throw 'AppointmentService.done: Unable to done an appointment=' +
                                JSON.stringify(appointment) + '. Err=' + err;
                        }
                        return result;
                    };
                ...
                this.cancel =
                    function (uuid) {
                        ...
                        try {
                            ...
                            throw 'AppointmentService.cancel: Unable to cancel an appointment=' +
                                JSON.stringify(appointment) + '. Err=' + err;
                        }
                        return result;
                    };
