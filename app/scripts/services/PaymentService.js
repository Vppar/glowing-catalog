(function(angular) {
	...
	entities.factory('CheckPayment', function CheckPayment(Payment) {
		...
		var service = function svc(amount, bank, agency, account, number,
				duedate) {
	...
	entities.factory('CreditCardPayment', function CreditCardPayment(Payment) {
		...
		var service = function svc(amount, flag, ccNumber, owner, ccDueDate,
				cvv, cpf, installments) {
	...
	angular
			...
			.service(
					'PaymentService',
					function PaymentService(ArrayUtils, Payment, CashPayment,
							CheckPayment, CreditCardPayment, ExchangePayment,
							CouponPayment) {
						...
						/**
						 * Returns the name of the instance of the given
						 * payment.
						 * 
						 * @param payment -
						 *            Payment to find the name.
						 */
						 ...
						 * 
						 * @param typeName -
						 *            Type of the payments that will be
						 *            returned.
						 * @throws Exception -
						 *             In case of an unknown payment type.
						 */
						var list = function list(typeName) {
							var paymentList = payments[typeName];
							if (!paymentList) {
								throw 'PaymentService.list: Unknown type of payment, typeName='
										+ typeName;
							}
						...
						var read = function read(typeName, id) {
							...
							var payment = ArrayUtils
									.find(paymentList, 'id', id);
							if (!payment) {
								throw 'PaymentService.read: Unknown payment instance, id='
										+ id;
							}
						 ...
						 * @throws Exception -
						 *             Throws an exception when the given
						...
						var update = function update(payment) {
							...
							var paymentInstance = ArrayUtils.find(paymentList,
									'id', payment.id);
							if (!paymentInstance) {
								throw 'PaymentService.update: Unknown payment instance, id='
										+ id;
							}
						...
						var remove = function remove(payment) {
							...
							var paymentInstance = ArrayUtils.find(paymentList,
									'id', payment.id);
							if (!paymentInstance) {
								throw 'PaymentService.remove: Unknown payment instance, id='
										+ payment.id;
							}
							// remove
							paymentList.splice(paymentList
									.indexOf(paymentInstance), 1);
						};
