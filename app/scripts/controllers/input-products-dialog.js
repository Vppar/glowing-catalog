    ...
    angular
        ...
        .controller(
            ...
            function ($scope, dialog, DataProvider, ArrayUtils, $filter) {
                ...
                $scope.removeFromStock =
                    function (product) {
                        var removedItem =
                            $filter('filter')($scope.productsToInsert, function (item) {
                        ...
                        for ( var idx in removedItem) {
                            filtratedProducts =
                                ($filter('filter')(filtratedProducts, function (product) {
                ...
                $scope.computeTotals =
                    function () {
                        ...
                        for ( var idx in $scope.productsToInsert) {
                            $scope.sumTotalPrice =
                                $scope.sumTotalPrice + $scope.productsToInsert[idx].price *
                                    $scope.productsToInsert[idx].quantity;
                            $scope.qtyTotal =
                                Number($scope.qtyTotal) +
