(function (angular) {
    ...
    angular.module('tnt.catalog.customer.add.emails', [
        ...
        [
            ...
            'dialog',
            'DialogService',
            function ($scope, $q, $filter, dialog, DialogService) {
                ...
                /**
                 * Function add - Verifies if entered email already exists in
                 * the $scope.emails array and if not, adds email to the last
                 * position of $scope.emails array
                 */
                ...
                $scope.confirm = function () {
                    ...
                    if ($scope.emails.length >= 1) {
                        emails = $scope.emails;
                        // We need to remove $$hashKey due a firebase error
                        if (emails) {
                            for ( var ix in emails) {
                                if (emails[ix].$$hashKey) {
                                    delete emails[ix].$$hashKey;
                                }
                            }
                        }
                    } else {
